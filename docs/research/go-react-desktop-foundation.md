...
- [x] Reactの再マウント後に実行中または完了済みの状態を復元できる（製品Reactテストによる代替確認）
...
PoCでは、イベントリスナーを解除した状態で操作を完了し、`GetOperationState`で終端状態へ
復旧できることを確認しました。

2026-08-30に、製品Reactテストの「再マウント時にApplicationの最新状態を復元する」で、
コンポーネントのアンマウントと再マウントを実行しました。このテストでは、アンマウント時の
イベント購読解除、再マウント時のイベント再登録、および`GetCurrentLaunchState`から同じ
Operation IDの実行中状態を復元できることを確認しています。

製品ReactテストはWails境界をモックしており、Windows実機での再マウントは未確認です。
